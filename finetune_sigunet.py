...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        run_name = f"{model_name}:{layers_count}-hidden_size:{hidden_size}-heads_count:{heads_count}-fixed:{pretrain_fixed}"
        ...
        trainer = finetuneSigunet(pretrained_checkpoint,\
                ...
                save_every, config, run_name, pretrain_fixed=pretrain_fixed)
        ...
        best_loss.append(trainer.best_val_metric)

        with open(f'models/finetune/{run_name}/best_model_config.json', 'w') as f:
            json.dump(best_model_config, f, indent=4, separators=(',', ': '))

    print(f'best_val_loss: {best_loss}')
